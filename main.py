...
def index():
    ...
    v_html = "".join([f"""<div class="card"><img src="{v["secure_url"].rsplit(".", 1)[0]}.jpg" style="width:100%;height:160px;object-fit:fill;background:#000;"><div style="padding:10px;"><b>{v["public_id"]}</b><br><br><div style="display:flex;gap:2px;"><a href="{v["secure_url"]}" class="btn btn-play" style="flex:1;text-align:center;">PLAY</a><a href="{v["secure_url"]}" download class="btn btn-dl" style="flex:1;text-align:center;">SAVE</a></div><div style="display:flex;gap:2px;margin-top:4px;"><a href="/modify?task=rename&pid={v["public_id"]}&type=video" class="btn btn-ren" style="flex:1;text-align:center;">NAME</a><a href="/modify?task=delete&pid={v["public_id"]}&type=video" class="btn btn-del" style="flex:1;text-align:center;">DEL</a></div></div></div>""" for v in videos])
    next_btn = f'<a href="/?next={new_c}&q={q}" class="btn" style="background:#333;width:92%;padding:15px;margin:10px;display:block;text-align:center;border-radius:10px;">LOAD NEXT ↓</a>' if new_c else ""
...
def pdf_home():
    ...
    f_list = "".join([f"""<div class="card" style="border-left:8px solid #e74c3c;padding:15px;"><b style="color:#c0392b;">{f["name"].upper()}</b><br><br><div style="display:flex;gap:5px;"><a href="/view_pdf?name={f["name"]}" class="btn btn-play" style="flex:3;text-align:center;">OPEN KITAB</a><a href="/modify?task=delete&pid={f["name"]}&type=pdf" class="btn btn-del" style="flex:1;text-align:center;">DEL</a></div></div>""" for f in folders if q in f["name"].lower()])
    return f'{STYLE}<div class="header" style="background:#c0392b;"><h2>PDF Archive</h2><a href="/" style="color:#fff;">← HOME</a><a href="/upload_pdf_page" class="btn btn-dl" style="margin-left:10px;">+ NEW</a></div><form class="search-box"><input name="q" placeholder="Search Kitab..." value="{q}"><button style="background:#c0392b;">SEARCH</button></form>{f_list}'
...
def view_pdf():
    ...
    h = "".join([f'<div class="card"><img src="{p["secure_url"]}" style="width:100%;"><div style="padding:10px;text-align:center;"><a href="{p["secure_url"]}" download class="btn btn-dl" style="width:80%;">SAVE PAGE</a></div></div>' for p in pages])
    next_btn = f"<a href='/view_pdf?name={name}&next={new_c}' class='btn btn-del' style='display:block;padding:18px;text-align:center;margin:10px;'>NEXT 10 PAGES →</a>" if new_c else ""
...
def ai_home():
    ...
    return f'{STYLE}<div class="header" style="background:#8e44ad;"><h2>AI Scanner</h2><a href="/" style="color:#fff;">← HOME</a></div><div class="card" style="padding:20px;text-align:center;"><form action="/enhance" method="POST" enctype="multipart/form-data"><input type="file" name="file" required><br><br><button class="btn" style="background:#8e44ad;width:100%;padding:15px;">SCAN & ENHANCE</button></form></div>{h_html}'
...
def modify():
    ...
    return render_template_string(f'{STYLE}<div class="header"><h2>Confirm</h2></div><div class="card" style="padding:20px;text-align:center;"><form action="/confirm" method="POST"><input type="hidden" name="pid" value="{{p}}"><input type="hidden" name="task" value="{{t}}"><input type="hidden" name="type" value="{{tp}}">{{% if t=="rename" %}}<input name="new" placeholder="New Name"><br>{{% endif %}}<input name="pw" type="password" placeholder="Pass"><br><br><button class="btn btn-del">OK</button></form></div>', t=t, p=p, tp=tp)
