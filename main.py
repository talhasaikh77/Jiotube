...
def login():
    ...
    return render_template_string('<body style="padding:20px;font-family:sans-serif;text-align:center;background:#eee;"><div style="background:#fff;padding:20px;border-radius:10px;box-shadow:0 2px 5px rgba(0,0,0,0.1);"><h3>AI Enhancer Login</h3><form method="POST"><input type="number" name="mobile" placeholder="Mobile No" style="width:100%;padding:15px;margin-bottom:10px;border:1px solid #ddd;border-radius:5px;"><br><input type="password" name="pw" placeholder="Password" style="width:100%;padding:15px;margin-bottom:15px;border:1px solid #ddd;border-radius:5px;"><br><button style="width:100%;padding:15px;background:#9b59b6;color:#fff;border:none;border-radius:5px;font-weight:bold;">LOGIN</button></form><p style="margin-top:15px;">New? <a href="/ai_register">Register Here</a></p><br><a href="/" style="text-decoration:none;color:#333;">← Back to Home</a></div></body>')

@app.route("/ai_register", methods=["GET", "POST"])
def register():
    if request.method == "POST":
        m = request.form.get("mobile"); p = request.form.get("pw")
        if not m or not p: return "Khali na chhodein! <a href='/ai_register'>Wapas</a>"
        try:
            if users_col.find_one({"mobile": m}): return "Number pehle se hai! <a href='/ai_login'>Login karein</a>"
            users_col.insert_one({"mobile": m, "password": hash_pw(p)})
            return 'Account ban gaya! <a href="/ai_login">Ab Login Karein</a>'
        except Exception as e:
            return f"Error: {e}"
    return render_template_string('<body style="padding:20px;font-family:sans-serif;text-align:center;background:#eee;"><div style="background:#fff;padding:20px;border-radius:10px;box-shadow:0 2px 5px rgba(0,0,0,0.1);"><h3>AI Register</h3><form method="POST"><input type="number" name="mobile" placeholder="Mobile No" style="width:100%;padding:15px;margin-bottom:10px;border:1px solid #ddd;border-radius:5px;"><br><input type="password" name="pw" placeholder="Set Password" style="width:100%;padding:15px;margin-bottom:15px;border:1px solid #ddd;border-radius:5px;"><br><button style="width:100%;padding:15px;background:#28a745;color:#fff;border:none;border-radius:5px;font-weight:bold;">CREATE ACCOUNT</button></form><p style="margin-top:15px;">Have account? <a href="/ai_login">Login</a></p></div></body>')

@app.route("/ai_home")
def ai_home():
    if 'user_id' not in session: return redirect(url_for('login'))
    return render_template_string('<body style="padding:20px;text-align:center;font-family:sans-serif;background:#eee;"><h3>AI Panel</h3><div style="display:grid;gap:10px;"><a href="/enhance_upload" style="background:#9b59b6;color:#fff;padding:20px;text-decoration:none;border-radius:10px;font-weight:bold;">+ ENHANCE PHOTO</a><a href="/ai_history" style="background:#333;color:#fff;padding:20px;text-decoration:none;border-radius:10px;font-weight:bold;">MY HISTORY</a><br><a href="/" style="color:#0078d7;font-weight:bold;text-decoration:none;">HOME</a> | <a href="/logout" style="color:red;font-weight:bold;text-decoration:none;">LOGOUT</a></div></body>')
...
def admin_upload(): return render_template_string('<body><h3>Admin Upload</h3><form action="/do_up" method="POST" enctype="multipart/form-data"><input type="file" name="file"><br><br><input type="text" name="name" placeholder="Name"><br><br><input type="password" name="pw" placeholder="Admin Pass"><br><br><button>UPLOAD</button></form></body>')
...
def modify():
    ...
    return render_template_string('<body style="text-align:center;padding:50px;"><form action="/confirm" method="POST"><input type="hidden" name="pid" value="{{p}}"><input type="hidden" name="task" value="{{t}}"><input type="hidden" name="type" value="{{tp}}">{% if t=="rename" %}<input type="text" name="new" placeholder="New Name"><br><br>{% endif %}<input type="password" name="pw" placeholder="Admin Pass"><br><br><button>CONFIRM</button></form></body>', t=t, p=p, tp=tp)
