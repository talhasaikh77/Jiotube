...
def pdf_home():
    q = request.args.get("q", "").strip().lower()
    try:
        # Fetching folders inside pdf_data
    ...
    if not f_html: f_html = "<div style='padding:20px;text-align:center;color:#666;'>No PDFs found. Wait a minute if you just uploaded.</div>"
...
def view_pdf():
    ...
    h = "".join([f"""<div style="background:#000;margin-bottom:15px;text-align:center;border-bottom:2px solid #e74c3c;"><img src="{p["secure_url"]}?v={ts}" style="width:100%;display:block;"><div style="padding:10px;background:#333;"><a href="{p["secure_url"].replace('/upload/','/upload/fl_attachment/')}" style="background:#28a745;color:#fff;text-decoration:none;padding:12px 25px;border-radius:5px;font-size:12px;font-weight:bold;">DOWNLOAD PAGE</a></div></div>""" for p in pages])
    p_btn = f"<a href='/view_pdf?name={name}&next={new_c}' style='display:block;background:#e74c3c;color:#fff;padding:18px;text-align:center;text-decoration:none;margin:10px;border-radius:5px;font-weight:bold;'>NEXT PAGES →</a>" if new_c else ""
...
def upload_pdf_page():
    ...
    <body style="padding:20px;font-family:sans-serif;text-align:center;background:#f0f0f0;">
        <div style="background:#fff;padding:25px;border-radius:12px;max-width:400px;margin:auto;">
            <h3 style="color:#e74c3c;">Nayi Kitab Upload</h3>
            <input type="file" id="f" style="width:100%;margin-bottom:20px;border:1px solid #ddd;padding:10px;border-radius:5px;"><br>
            <input type="text" id="n" placeholder="Kitab Ka Naam" style="width:90%;padding:12px;margin-bottom:10px;border:1px solid #ddd;border-radius:6px;"><br>
            <input type="password" id="p" placeholder="Password" style="width:90%;padding:12px;margin-bottom:20px;border:1px solid #ddd;border-radius:6px;"><br>
            <div id="progBox" style="display:none;margin-bottom:20px;">
                <div style="background:#eee;height:20px;border-radius:10px;overflow:hidden;"><div id="bar" style="width:0%;height:100%;background:#e74c3c;"></div></div>
                <small id="stat">Sending...</small>
            </div>
            <button onclick="startUp()" id="btn" style="width:100%;padding:18px;background:#e74c3c;color:#fff;border:none;border-radius:8px;font-weight:bold;">UPLOAD</button>
        </div>
        <script>
        function startUp(){
            ...
            xhr.upload.onprogress = function(e){
                ...
                document.getElementById('stat').innerText="Upload: "+p+"%";
            };
            xhr.onload = function(){ 
                if(xhr.status==200){ 
                    alert("Upload Success! Wait 1 min for list to update.");
