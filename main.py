...
def view_pdf():
    ...
    h = "".join([f"""<div style="margin-bottom:15px;background:#fff;text-align:center;"><img src="{p["secure_url"]}?v={ts}" style="width:100%;display:block;min-height:200px;background:#eee;"><div style="padding:10px;"><a href="{p["secure_url"].replace("/upload/","/upload/fl_attachment/")}" style="color:#fff;text-decoration:none;font-size:12px;background:#28a745;padding:5px 10px;border-radius:4px;">DOWNLOAD HQ</a></div></div>""" for p in pages])
...
def do_pdf_upload():
    if request.form.get("pw") == ADMIN_PASSWORD:
        ...
        if f:
            ...
            f.save(pdf_path)
            # Yahan background thread start ho rahi hai taaki upload fauran "OK" bol de
