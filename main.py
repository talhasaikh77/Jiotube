...
def view_pdf():
    ...
    h = "".join([f"""<div style="background:#000;margin-bottom:15px;text-align:center;border-bottom:3px solid #e74c3c;"><img src="{p["secure_url"]}?v={ts}" style="width:100%;display:block;"><div style="padding:10px;background:#333;"><a href="{p["secure_url"].rsplit(".", 1)[0]}.jpg" style="background:#28a745;color:#fff;font-size:11px;text-decoration:none;padding:10px 20px;border-radius:5px;font-weight:bold;">DOWNLOAD PAGE</a></div></div>""" for p in pages])
    p_btn = f"<a href='/view_pdf?name={name}&next={new_c}' style='display:block;background:#e74c3c;color:#fff;padding:18px;text-align:center;text-decoration:none;font-weight:bold;font-size:14px;border-radius:5px;margin:10px;'>LOAD NEXT 10 PAGES →</a>" if new_c else ""
...
def upload_pdf_page():
    ...
    <body style="padding:20px;font-family:sans-serif;text-align:center;background:#f0f0f0;">
        <div style="background:#fff;padding:20px;border-radius:10px;max-width:400px;margin:auto;">
            <h3 style="color:#e74c3c;">PDF Direct Upload</h3>
            <p style="font-size:11px;color:#666;">Is baar 28% par nahi atkega!</p>
            <input type="file" id="f" style="width:100%;margin-bottom:15px;"><br>
            <input type="text" id="n" placeholder="Book Name" style="width:90%;padding:12px;margin-bottom:10px;border:1px solid #ddd;border-radius:5px;"><br>
            <input type="password" id="p" placeholder="Pass" style="width:90%;padding:12px;margin-bottom:15px;border:1px solid #ddd;border-radius:5px;"><br>
            <div id="progBox" style="display:none;margin-bottom:15px;">
                <div style="background:#eee;height:25px;border-radius:10px;overflow:hidden;"><div id="bar" style="width:0%;height:100%;background:#e74c3c;"></div></div>
                <small id="stat">Connecting...</small>
            </div>
            <button onclick="up()" id="btn" style="width:100%;padding:15px;background:#e74c3c;color:#fff;border:none;border-radius:5px;font-weight:bold;">UPLOAD NOW</button>
        </div>
        <script>
        function up(){
            ...
            xhr.onload = function(){ if(xhr.status==200){ location.href="/pdf_home"; } else { alert("Error!"); } };
            xhr.send(fd);
        }
        </script>
    </body>
...
def do_pdf_upload():
    if request.form.get("pw") == ADMIN_PASSWORD:
        ...
        if f:
            ...
            f.save(pdf_path) # Now server is better at handling temporary files
...
def modify():
    ...
    return render_template_string("""<body style="text-align:center;padding:25px;font-family:sans-serif;"><div style="background:#fff;padding:20px;border-radius:10px;border:1px solid #ddd;"><h4>{{t.upper()}}</h4><form action="/confirm" method="POST"><input type="hidden" name="pid" value="{{p}}"><input type="hidden" name="task" value="{{t}}"><input type="hidden" name="type" value="{{tp}}">{% if t=="rename" %}<input type="text" name="new" placeholder="Enter New Name" style="width:90%;padding:12px;margin-bottom:15px;border:1px solid #ddd;border-radius:5px;"><br>{% endif %}<input type="password" name="pw" placeholder="Pass" style="width:90%;padding:12px;margin-bottom:20px;border:1px solid #ddd;border-radius:5px;"><br><button style="width:100%;padding:15px;background:#333;color:#fff;border:none;border-radius:5px;font-weight:bold;">CONFIRM</button></form></div></body>""", t=t, p=p, tp=tp)
...
def admin_upload():
    ...
    <body style="padding:20px;text-align:center;font-family:sans-serif;">
        ...
        <form action="/do_up" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="name" placeholder="Name" style="width:80%;padding:10px;margin-bottom:10px;"><br>
