...
def process_pdf_fast(file, pdf_name):
    try:
        ...
        for i in range(len(doc)):
            ...
            # Image matrix ko optimize kiya taaki jaldi load ho
...
def view_pdf():
    ...
    h = "".join([f"""<div style="margin-bottom:15px;background:#000;min-height:200px;"><img src="{p["secure_url"]}" style="width:100%;" loading="lazy"><div style="padding:10px;text-align:center;"><a href="{p["secure_url"].replace("/upload/","/upload/fl_attachment/")}" style="color:#fff;text-decoration:none;font-size:12px;background:#28a745;padding:5px 10px;border-radius:4px;">DOWNLOAD HQ</a></div></div>""" for p in pages])
