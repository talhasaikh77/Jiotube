...
app = Flask(__name__)
app.secret_key = "jio_stable_v37_thumbnail_fixed"
...
def index():
    ...
    <div class="card">
        <div class="thumb-container">
            <img src="{v["secure_url"].rsplit(".", 1)[0]}.jpg" class="thumb-img" onerror="this.src='https://res.cloudinary.com/dawterffe/video/upload/c_fill,h_180,w_320,so_0/{v["public_id"]}.jpg'">
        </div>
        <div style="padding:10px;">
            ...
            <div style="display:flex; gap:5px; margin-top:10px;">
                ...
                <a href="/modify?task=delete&pid={v["public_id"]}&type=video" class="btn btn-danger">DEL</a>
            </div>
        </div>
    </div>''' for v in videos])
...
def pdf_home():
    ...
    f_list = "".join([f'''<div class="card"><div style="padding:15px;"><b>{f["name"].split("__")[0].upper()}</b><div style="display:flex; gap:5px; margin-top:10px;"><a href="/view_pdf?name={f["name"]}" class="btn btn-jio" style="flex:2;">OPEN</a><a href="/modify?task=rename&pid={f["name"]}&type=pdf" class="btn btn-ren">NAME</a><a href="/modify?task=delete&pid={f["name"]}&type=pdf" class="btn btn-danger">DEL</a></div></div></div>''' for f in folders if q in f["name"].lower()])
...
def ai_chatter():
    ...
    if request.method == "POST":
        ...
        resp = f"Joya: Atif bhai, aapne pucha '{msg}'. Main aapki help ke liye hamesha taiyar hoon! (Saved in background)"
