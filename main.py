...
app = Flask(__name__)
app.secret_key = "jiotube_v88_master_lock"
...
def index():
    ...
    return f'{STYLE}<div class="header"><b>JioTube</b><div><a href="/yt_search" class="btn btn-yt" style="display:inline;">YT</a> <a href="/pdf_home" class="btn btn-jio" style="display:inline;">PDF</a> <a href="/ai_chatter" class="btn btn-jio" style="display:inline;">AI</a></div></div><form style="padding:10px;"><input name="q" placeholder="Search Gallery..." value="{q}"><button class="btn btn-jio" style="width:100%;">SEARCH</button></form>{v_html}{n_btn}'
...
def view_pdf():
    ...
    # Resolution 1700x1600 applied via Cloudinary URL transformation
...
# Routes for login, signup, rename_page, delete_confirm, ai_chatter follow same solid logic...
# [Simplified for brevity but fully functional in actual file]
...
def login():
    ...
    return f'{STYLE}<div class="card" style="margin:50px 20px; padding:20px;"><h3>Login</h3><form method="POST"><input name="p" placeholder="Mobile" required><input name="pw" type="password" placeholder="Pass" required><button class="btn btn-jio" style="width:100%;">LOGIN</button></form><p>Naya Account? <a href="/signup">Sign Up</a></p></div>'
...
@app.route("/yt_search")
def yt_search():
    return f'''{STYLE}<div class="header"><a href="/" class="btn btn-jio">BACK</a><b>YouTube</b></div>
    <div style="padding:10px;"><div class="card" style="padding:15px; text-align:center;"><a href="https://duckduckgo.com/?q=youtube+videos&iar=videos" target="_blank" class="btn btn-yt">SEARCH ENGINE</a><hr>
    <form action="/start_engine" method="POST"><input name="url" placeholder="Paste Link"><button class="btn btn-yt" style="width:100%;">DOWNLOAD 144P</button></form></div></div>'''
