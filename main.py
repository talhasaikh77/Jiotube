...
app = Flask(__name__)
app.secret_key = "jio_full_v36_white_stable"
...
STYLE = """<style>
    ...
    .card { background: var(--card); margin:10px; border-radius:8px; overflow:hidden; border: 1px solid var(--border); box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
    ...
    .btn-jio { background: var(--jio-blue); } .btn-ren { background:#ff9900; } .btn-danger { background:#e50914; } .btn-outline { background:transparent; border:1px solid #fff; color:#fff; }
    .btn-dark-outline { background:transparent; border:1px solid var(--jio-blue); color:var(--jio-blue); }
    .search-box { background: #fff; display:flex; margin:10px; border-radius:8px; overflow:hidden; border: 1px solid var(--border); }
    .search-box input { flex:1; border:none; padding:10px; outline:none; font-size:14px; }
    .search-box button { background: var(--jio-blue); color:#fff; border:none; padding:0 15px; }
    .thumb-med { width:100%; height:160px; object-fit:cover; background:#eee; }
    ...
    .m-u { align-self:flex-end; background:#e1f5fe; color:#01579b; padding:10px; border-radius:12px 12px 0 12px; max-width:80%; font-size:13px; border:1px solid #b3e5fc; }
    .m-ai { align-self:flex-start; background:#f5f5f5; color:#333; padding:10px; border-radius:12px 12px 12px 0; max-width:80%; font-size:13px; border:1px solid #e0e0e0; }
</style>"""
...
def index():
    ...
    nb = f'<a href="/video_home?next={new_c}&q={q}" class="btn btn-dark-outline" style="display:block; margin:10px; text-align:center;">LOAD NEXT</a>' if new_c else ""
    return f'{STYLE}<div class="header"><a href="/" class="logo">JioTube</a><div><a href="/pdf_home" class="btn btn-outline" style="padding:6px 10px;">PDF</a> <a href="/ai_home" class="btn btn-outline" style="padding:6px 10px; background:rgba(255,255,255,0.2);">AI</a></div></div><form class="search-box"><input name="q" placeholder="Search videos..." value="{q}"><button>GO</button></form><div style="padding:0 10px;"><a href="/admin_upload" class="btn btn-jio" style="width:100%; text-align:center;">+ UPLOAD VIDEO</a></div>{v_html}{nb}'
...
def pdf_home():
    ...
    return f'{STYLE}<div class="header"><a href="/video_home" class="btn btn-outline">TUBE</a><b class="logo">JioPDF</b><a href="/ai_home" class="btn btn-outline">AI</a></div><form class="search-box"><input name="q" placeholder="Search books..." value="{q}"><button>FIND</button></form><div style="padding:0 10px;"><a href="/upload_pdf_page" class="btn btn-jio" style="width:100%; text-align:center;">+ NEW BOOK</a></div>{f_list}'

@app.route("/ai_home")
def ai_home():
    if 'u' not in session: return redirect(url_for('ai_auth'))
    return f'{STYLE}<div class="header"><a href="/" class="btn btn-outline">HOME</a><b class="logo">JioAI</b><a href="/logout" class="btn btn-danger">LOGOUT</a></div><div style="padding:20px; text-align:center;"><div class="card" style="padding:20px;"><a href="/ai_enhance" class="btn btn-jio" style="width:100%; margin-bottom:10px; padding:15px;">AI PHOTO ENHANCE</a><a href="/ai_chatter" class="btn btn-dark-outline" style="width:100%; padding:15px;">AI CHATTER (JOYA)</a></div></div>'
...
def ai_chatter():
    ...
    if request.method == "POST":
        ...
        # Instant Response Logic for Atif bhai
        ai_resp = f"Atif bhai, Joya yahan hai! Aapka sawaal: '{msg}'. Jawab: Main ek advance AI assistant hoon jo aapki files aur queries handle karne ke liye bani hoon. Aur kuch pucha?"
    ...
    <div style="padding:10px; background:#eee; position:fixed; bottom:0; width:100%; box-sizing:border-box; border-top:1px solid #ddd;">
        <form method="POST" style="display:flex; gap:8px;">
            <input name="msg" style="flex:1; padding:10px; border-radius:8px; border:1px solid #ccc;" placeholder="Type your question..." required>
            <button class="btn btn-jio">ASK</button>
        </form>
...
def ai_enhance():
    ...
    h_html = "".join([f'<div class="card"><img src="{i["url"]}" class="thumb-med"><div style="padding:10px;text-align:center;"><a href="{i["url"]}" download class="btn btn-jio">DOWNLOAD</a></div></div>' for i in history])
