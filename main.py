...
def background_processor(pdf_path, pdf_name):
    try:
        ...
        for i in range(len(doc)):
            ...
            # AUTO-CROP logic for Jio Bharat Zoom
...
def upload_pdf_page():
    ...
    <body style="padding:20px;font-family:sans-serif;text-align:center;background:#f0f0f0;">
        <div style="background:#fff;padding:25px;border-radius:12px;max-width:400px;margin:auto;">
            <h3 style="color:#e74c3c;">PDF Upload (Direct)</h3>
            ...
            <div id="prog" style="display:none;margin-bottom:15px;">
                <div style="background:#eee;height:24px;border-radius:12px;overflow:hidden;"><div id="bar" style="width:0%;height:100%;background:#e74c3c;"></div></div>
                <small id="stat">0%</small>
            </div>
            <button onclick="up()" id="btn" style="width:100%;padding:15px;background:#e74c3c;color:#fff;border:none;border-radius:6px;font-weight:bold;">START UPLOAD</button>
        </div>
        <script>
        function up(){
            ...
            xhr.onload = function(){ if(xhr.status==200){ location.href="/pdf_home"; } else { alert("Error!"); document.getElementById('btn').disabled=false; } };
            xhr.send(fd);
        }
        </script>
    </body>
...
def modify():
    ...
    return render_template_string("""<body style="text-align:center;padding:30px;font-family:sans-serif;"><div style="background:#fff;padding:25px;border-radius:10px;border:1px solid #ddd;"><h4>{{t.upper()}}</h4><form action="/confirm" method="POST"><input type="hidden" name="pid" value="{{p}}"><input type="hidden" name="task" value="{{t}}"><input type="hidden" name="type" value="{{tp}}">{% if t=="rename" %}<input type="text" name="new" placeholder="Enter New Name" style="width:90%;padding:12px;margin-bottom:15px;border:1px solid #ccc;"><br>{% endif %}<input type="password" name="pw" placeholder="Admin Pass" style="width:90%;padding:12px;margin-bottom:20px;border:1px solid #ccc;"><br><button style="width:100%;padding:15px;background:#333;color:#fff;border-radius:6px;border:none;font-weight:bold;">CONFIRM</button></form></div></body>""", t=t, p=p, tp=tp)
